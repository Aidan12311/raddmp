...
/* Full-screen reactive glow behind everything. Reacts to audio when playing,
   drifts gently (non-deterministic) when idle. */
export function AmbientBackground({ getAnalyser, playing }) {
  ...
  useEffect(() => {
    ...
    if (!st.current) st.current = { t: 0, sm: [0, 0, 0, 0], flick: [0, 0, 0, 0], wx: [0, 0, 0, 0], wy: [0, 0, 0, 0], tx: [0, 0, 0, 0], ty: [0, 0, 0, 0] };
    const s = st.current;
    const draw = () => {
...
export function Spectrum({ getAnalyser, playing, big, colors = BRAND }) {
  const cvs = useRef(null); const raf = useRef(0);
  useEffect(() => {
    const canvas = cvs.current; if (!canvas) return;
    const ctx = canvas.getContext("2d");
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    let phase = 0; const [g1, g2] = colors;
    const draw = () => {
...
function roundRect(ctx, x, y, w, h, r) { r = Math.max(0, Math.min(r, Math.abs(w) / 2, Math.abs(h) / 2)); ctx.beginPath(); ctx.moveTo(x + r, y); ctx.arcTo(x + w, y, x + w, y + h, r); ctx.arcTo(x + w, y + h, x, y + h, r); ctx.arcTo(x, y + h, x, y, r); ctx.arcTo(x, y, x + w, y, r); ctx.closePath(); }

export function EQCurve({ gains, colors = BRAND }) {
  const W = 260, H = 64, pad = 6; const [g1, g2] = colors; const pts = []; const n = 60;
  for (let i = 0; i <= n; i++) { const t = i / n; let v = 0; EQ_BANDS.forEach((b, bi) => { const bx = Math.log10(b.hz / 40) / Math.log10(16000 / 40); v += gains[bi] * Math.exp(-Math.pow((t - bx) / 0.16, 2)); }); const y = H / 2 - (v / 12) * (H / 2 - pad); pts.push([pad + t * (W - pad * 2), Math.max(pad, Math.min(H - pad, y))]); }
